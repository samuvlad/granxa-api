...
- **Non hai ferramentas de lint nin typecheck configuradas.** Non hai config de ruff/mypy/pyright, non hai pre-commit. `pytest` é a única verificación. Non corras un `make lint` — non existe.
- **CI e despregue**: `.github/workflows/api.yml` constrúe `Dockerfile.prod` (sen extras `[dev]`) e publica en GHCR (`ghcr.io/samuvlad/granxa-api:master`) en cada push a `master`. O despregue prod vive en `deploy/` (`docker-compose.prod.yml`, `deploy.sh`, `backup.sh`, `restore.sh`, `README.md`) e corre na Raspberry Pi (`/opt/granxa`). En prod o `entrypoint.sh` arranca uvicorn **sen** `--reload` (`RELOAD=1` só no compose de dev). CORS configúrase con `CORS_ORIGINS` (env, separado por comas).
